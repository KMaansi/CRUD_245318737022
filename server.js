...
app.get('/', (req,res)=>{                
	...
	res.render('boot.ejs', {data:result})      
	})
})
...
app.post('/AddData', (req,res)=>{			
	db.collection('women').save(req.body, (err,result)=>{			
		if(err) return console.log(err)
	res.redirect('/')                        
	})
})
...
app.post('/update', (req,res)=>{			
		db.collection('women').find().toArray((err,result)=>{	   			
			...
			db.collection('women').findOneAndUpdate({pid:req.body.id},{
...
app.post('/delete',(req,res)=>{
	db.collection('women').findOneAndDelete({pid:req.body.id}, (err,result)=>{    
		if(err)
			return console.log(err)
		res.redirect('/')
	})
